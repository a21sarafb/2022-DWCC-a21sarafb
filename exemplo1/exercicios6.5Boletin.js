...
function jugadores() {
	let numAleatorio = numeroAleatorio(1, 2);
	jugando(numAleatorio);
	if (numAleatorio == 1) {
		document.getElementById("tocalle").innerHTML =
			document.getElementById("tocalle").innerHTML + "Usuario";
	} else {
		document.getElementById("tocalle").innerHTML =
			document.getElementById("tocalle").innerHTML + "Ordenador";
	}
}
...
function jugar() {
	if (eleccion == 2) {
		// casos en los que empieza el ordenador
		if (jugador != eleccion) {
			if (final() == 9) {
				fcelda(8);
			} else {
...
function pregunta() {
	document.getElementById("final").innerHTML =
		'<button id="pregunta" type="button" onclick="resetear()" class="btn btn-warning">Volver a jugar</button>';
}
